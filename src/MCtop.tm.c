...
# line 321 "/Users/vicent/GitHubProjects/MCtop/src/MCtop.tm"
#include "mathlink.h"
#include "ftypes.h"
#include <stdio.h>
#include <unistd.h>
...
# line 775 "/Users/vicent/GitHubProjects/MCtop/src/MCtop.tm.c"
...
void eshape P(( double _tp1, double _tp2, double _tp3, double _tp4));
...
void stabledistributions P(( double _tp1, double _tp2, const char * _tp3, const char * _tp4, int _tp5, int _tp6, int _tp7, int _tp8));
...
#endif
{
	...
	stabledistributions(_tp1, _tp2, _tp3, _tp4, _tp5, _tp6, _tp7, _tp8);

	res = 1;
L8: L7: L6: L5: L4:	MLReleaseString(mlp, _tp4);
...
static struct func {
	...
	const char  *f_name;
		...
		{ 2, 0, _tr2, "esmin" },
		{ 2, 0, _tr3, "esmax" },
		{ 1, 0, _tr4, "cparamcomputer" },
		{ 7, 0, _tr5, "cparamlist" },
		{ 2, 0, _tr6, "legendrelist" },
		{14, 0, _tr7, "cparamdistribution" },
		{15, 0, _tr8, "cparamlegendredistro" },
		{14, 0, _tr9, "cparamlegendre" },
		{13, 0, _tr10, "esdistributions" },
		{ 8, 0, _tr11, "stabledistributions" },
		{13, 0, _tr12, "eslegendre" },
		{ 7, 0, _tr13, "eslist" },
		{ 3, 0, _tr14, "esliststable" },
		{ 1, 0, _tr15, "escomputer" },
		{ 5, 0, _tr16, "esminmax4" },
		{ 5, 0, _tr17, "cparammaxmin4" },
		{ 5, 0, _tr18, "esmaxmin4" },
		{ 5, 0, _tr19, "esmaxmin6" },
		{ 5, 0, _tr20, "cparammaxmin6" },
		{ 5, 0, _tr21, "cparamminmax4" },
		{ 5, 0, _tr22, "cparamminmax6" },
		{ 5, 0, _tr23, "esminmax6" },
		{ 5, 0, _tr24, "vectors4" },
		{ 5, 0, _tr25, "cparam4" },
		{ 5, 0, _tr26, "cparam6" },
		{ 5, 0, _tr27, "cparambeta4" },
		{ 5, 0, _tr28, "cparambeta6" },
		{ 5, 0, _tr29, "vectors6" },
		{ 5, 0, _tr30, "restvectors4" },
		{ 5, 0, _tr31, "restvectors6" }
		};
...
static const char* evalstrs[] = {
	...
	(const char*)0,
	"EShape::usage = \"EShape[m, Q, h1, h2] computes the value of the ",
	"event shape\"",
	(const char*)0,
	...
	"imum and minimum value for Event shapes with 4-particle final st",
	"ate\"",
	(const char*)0,
	"ESMinMax6::usage = \"ESMinMax6[n, mt, mb, mW, Q] computes the max",
	"imum and minimum value for Event shapes with 6-particle final st",
	"ate\"",
	(const char*)0,
	"CparamMinMax4::usage = \"CparamMinMax4[n, mt, mb, mW, Q] computes",
	" the maximum and minimum value for the C-parameter Event shape w",
	"ith 4-particle final state\"",
	(const char*)0,
	"CparamMinMax6::usage = \"CparamMinMax6[n, mt, mb, mW, Q] computes",
	" the maximum and minimum value for the C-parameter Event shape w",
	"ith 6-particle final state\"",
	(const char*)0,
	"CparamMaxMin4::usage = \"CparamMaxMin4[eps, mt, mb, mW, Q] comput",
	"es the maximum and minimim values and locations for the C-parame",
	"ter Event shape with 4-particle final state\"",
	(const char*)0,
	"CparamMaxMin6::usage = \"CparamMaxMin6[eps, mt, mb, mW, Q] comput",
	"es the maximum and minimim values and locations for the C-parame",
	"ter Event shape with 6-particle final state\"",
	(const char*)0,
	"ESMaxMin6::usage = \"ESMaxMin6[eps, mt, mb, mW, Q] computes the m",
	"aximum and minimim values and locations for Event shapes with 4-",
	"particle final state\"",
	(const char*)0,
	"ESMaxMin4::usage = \"ESMaxMin4[eps, mt, mb, mW, Q] computes the m",
	"aximum and minimim values and locations for Event shapes with 6-",
	"particle final state\"",
	(const char*)0,
	...
	"d, method, Nlin, Nlog, Nevent, Niter] computes the distribution ",
...
#endif
{
	...
	if (_res) _res = _definepattern(mlp, (char *)"CparamComputer[p_]", (char *)"{Flatten[Transpose[p]]}", 4);
	if (_res) _res = _definepattern(mlp, (char *)"CparamList[mt_, mb_, mW_, Q_, Cmin_, Cmax_, Nbins_]", (char *)"{mt, mb, mW, Q, Cmin, Cmax, Nbins}", 5);
	if (_res) _res = _definepattern(mlp, (char *)"LegendreList[n_, x_]", (char *)"{n, x}", 6);
	if (_res) _res = _definepattern(mlp, (char *)"CparamDistribution[mt_, mb_, mW_, Q_, expand_, method_, spin_, decay_,                  current_, Cmin_, Cmax_, Nbins_, Nevent_, Niter_]", (char *)"{mt, mb, mW, Q, expand, method, spin, decay, current, Cmin, Cmax, Nbins,                  Nevent, Niter}", 7);
	...
	if (_res) _res = _definepattern(mlp, (char *)"StableDistributions[mt_, Q_, oriented_, method_, Nlin_,                  Nlog_, Nevent_, Niter_]", (char *)"{mt, Q, oriented, method, Nlin, Nlog, Nevent, Niter}", 11);
	if (_res) _res = _definepattern(mlp, (char *)"ESLegendre[mt_, mb_, mW_, Q_, method_, spin_, decay_, current_,                  Cmin_, Cmax_, n_, Nevent_, Niter_]", (char *)"{mt, mb, mW, Q, method, spin, decay, current, Cmin, Cmax, n, Nevent, Niter}", 12);
	if (_res) _res = _definepattern(mlp, (char *)"ESList[mt_, mb_, mW_, Q_, ESmin_, ESmax_, Nbins_]", (char *)"{mt, mb, mW, Q, ESmin, ESmax, Nbins}", 13);
	if (_res) _res = _definepattern(mlp, (char *)"ESListStable[mt_, Q_, Nbins_]", (char *)"{mt, Q, Nbins}", 14);
	if (_res) _res = _definepattern(mlp, (char *)"EScomputer[p_]", (char *)"{Flatten[Transpose[p]]}", 15);
	if (_res) _res = _definepattern(mlp, (char *)"ESMinMax4[n_, mt_, mb_, mW_, Q_]", (char *)"{n, mt, mb, mW, Q}", 16);
	if (_res) _res = _definepattern(mlp, (char *)"CparamMaxMin4[eps_, mt_, mb_, mW_, Q_]", (char *)"{eps, mt, mb, mW, Q}", 17);
	if (_res) _res = _definepattern(mlp, (char *)"ESMaxMin4[eps_, mt_, mb_, mW_, Q_]", (char *)"{eps, mt, mb, mW, Q}", 18);
	if (_res) _res = _definepattern(mlp, (char *)"ESMaxMin6[eps_, mt_, mb_, mW_, Q_]", (char *)"{eps, mt, mb, mW, Q}", 19);
	if (_res) _res = _definepattern(mlp, (char *)"CparamMaxMin6[eps_, mt_, mb_, mW_, Q_]", (char *)"{eps, mt, mb, mW, Q}", 20);
	if (_res) _res = _definepattern(mlp, (char *)"CparamMinMax4[n_, mt_, mb_, mW_, Q_]", (char *)"{n, mt, mb, mW, Q}", 21);
	if (_res) _res = _definepattern(mlp, (char *)"CparamMinMax6[n_, mt_, mb_, mW_, Q_]", (char *)"{n, mt, mb, mW, Q}", 22);
	if (_res) _res = _definepattern(mlp, (char *)"ESMinMax6[n_, mt_, mb_, mW_, Q_]", (char *)"{n, mt, mb, mW, Q}", 23);
	if (_res) _res = _definepattern(mlp, (char *)"Vectors4[x_, mt_, mb_, mW_, Q_]", (char *)"{x, mt, mb, mW, Q}", 24);
	if (_res) _res = _definepattern(mlp, (char *)"Cparam4[x_, mt_, mb_, mW_, Q_]", (char *)"{x, mt, mb, mW, Q}", 25);
	if (_res) _res = _definepattern(mlp, (char *)"Cparam6[x_, mt_, mb_, mW_, Q_]", (char *)"{x, mt, mb, mW, Q}", 26);
	if (_res) _res = _definepattern(mlp, (char *)"CparamBeta4[x_, mt_, mb_, mW_, Q_]", (char *)"{x, mt, mb, mW, Q}", 27);
	if (_res) _res = _definepattern(mlp, (char *)"CparamBeta6[x_, mt_, mb_, mW_, Q_]", (char *)"{x, mt, mb, mW, Q}", 28);
...
#endif
{
	return _MLDoCallPacket( mlp, _tramps, 32);
} /* MLDoCallPacket */
